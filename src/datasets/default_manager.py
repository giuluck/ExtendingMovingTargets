...
class DefaultManager(AbstractManager):
    ...
    FEATURES: Dict[str, FeatureInfo] = {
        'default': FeatureInfo(dtype='float', alias=None),
        'LIMIT_BAL': FeatureInfo(dtype='float', alias='limit'),
        'SEX': FeatureInfo(dtype='category', alias='sex'),
        'EDUCATION': FeatureInfo(dtype='category', alias='education'),
        'MARRIAGE': FeatureInfo(dtype='category', alias='marriage'),
        'AGE': FeatureInfo(dtype='float', alias='age'),
        'PAY_0': FeatureInfo(dtype='float', alias='sep_status'),
        'PAY_2': FeatureInfo(dtype='float', alias='aug_status'),
        'PAY_3': FeatureInfo(dtype='float', alias='jul_status'),
        'PAY_4': FeatureInfo(dtype='float', alias='jun_status'),
        'PAY_5': FeatureInfo(dtype='float', alias='may_status'),
        'PAY_6': FeatureInfo(dtype='float', alias='apr_status'),
        'BILL_AMT1': FeatureInfo(dtype='float', alias='sep_bill'),
        'BILL_AMT2': FeatureInfo(dtype='float', alias='aug_bill'),
        'BILL_AMT3': FeatureInfo(dtype='float', alias='jul_bill'),
        'BILL_AMT4': FeatureInfo(dtype='float', alias='jun_bill'),
        'BILL_AMT5': FeatureInfo(dtype='float', alias='may_bill'),
        'BILL_AMT6': FeatureInfo(dtype='float', alias='apr_bill'),
        'PAY_AMT1': FeatureInfo(dtype='float', alias='sep_payment'),
        'PAY_AMT2': FeatureInfo(dtype='float', alias='aug_payment'),
        'PAY_AMT3': FeatureInfo(dtype='float', alias='jul_payment'),
        'PAY_AMT4': FeatureInfo(dtype='float', alias='jun_payment'),
        'PAY_AMT5': FeatureInfo(dtype='float', alias='may_payment'),
        'PAY_AMT6': FeatureInfo(dtype='float', alias='apr_payment')
    }
    """The default dataset features."""

    MARKERS = {k: v for k, v in enumerate(['o', 's', '^', '+'])}
    """The ordered dictionary of markers used to plot data."""
    ...
    def __init__(self, filepath: str, full_features: bool = False, full_grid: bool = False, grid_augmented: int = 10,
                 grid_ground: Optional[int] = None, x_scaling: str = 'std', train_fraction: float = 0.8):
        """
        :param filepath:
        ...
        """
        self.months = None
        """The monthly features considered (they are the monotonic ones)."""

        grid = None
        if full_features:
            assert full_grid is False, "'full_grid' is not supported with 'full_features'"
            self.months = ['sep', 'aug', 'jul', 'jun', 'may', 'apr']
            x_scaling = {v.alias or k: x_scaling for k, v in DefaultManager.FEATURES.items() if v.dtype == 'float'}
        else:
            self.months = ['sep']
    ...
    def _summary_plot(self, model, figsize: Figsize, tight_layout: TightLayout, **additional_kwargs):
        ...
        marriage, status = np.meshgrid([0, 1], np.arange(-2, 9))
        grid = pd.DataFrame.from_dict({'marriage': marriage.flatten(), 'sep_status': status.flatten()})
        y = model.predict(grid).flatten()
        sns.lineplot(data=grid, x='sep_status', y=y, hue='marriage').set(
